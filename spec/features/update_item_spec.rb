...
feature "Updating an item in the inventory" do
  ...
  scenario "should be possible by an admin" do
    ...
    expect(item.location).to_not eq new_location
    ...
    expect(find_field("item_location").value).to eq item.location

    fill_in "item_location", with: new_location
    click_button "Save"

    expect(page).to have_content("#{item.title} was succesfully updated!")
    expect(find_field("item_location").value).to eq new_location
  end
  scenario "should be possible by an item owner" do
    item = items(:macbook2)
